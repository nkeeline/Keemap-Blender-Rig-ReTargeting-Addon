...
Blender Rig Retargeting Addon allows mapping motions of one rig to another.  Works with ANY rig and allows user to map bones from one rig to another and save mapping files out to hard drive.  This script is tested and working on blender 3.3.1, if a newer version of blender breaks the script I'm sorry.

This script enables transferring from an imported rig to a target rig position ID bones with offsets.  It gives you (as much as I can) COMPLETE control over the transfer as well as allowing you to customize the transfer.  This script all to position IK Target bones from FK transfers,  this leaves the destination rig in animation friendly and tweakable state.  You can position the rig with one click at the desired keyframe with the 'Test All' button.  You can then scroll forward and do it again making keyframes as you go at exactly the right times.  This enables you to transfer and only have keyframes exactly where you want them on the target bones used by animators only.  It is meant to automate the hand process of matching a target rig to a source rig even if they are rigged entirely differently.  This tool is being used in a film pipeline and is meant to allow animators to do exactly what they need and no more with the control they need.  We have used this on several films and are evolving as the films get better.  We have addressed some community issues expressed on github and appreciate the feedback.
